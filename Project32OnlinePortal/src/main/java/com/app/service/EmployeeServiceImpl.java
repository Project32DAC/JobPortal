...
//		" e.degreeMarks= :degreeM"+" e.branch= :br"+ " e.experience= :exp"+  " e.contact= :cn"+ " e.skills= :sk"; 
	...
	public ResumeResponseDTO generateResume(long empId) {
		// String jpql="Select u.firstName,u.lastName,u.email,e.sscMarks,e.hscMarks,e.degreeMarks,e.graduation,e.branch,e.experience,e.contact,e.skills from UserEntity u join fetch u.emp e where u.id=:eid";
...
//		return null;}).collect(Collectors.toList()); 
		...
		//List<Job>jobs = jobRepo.findAll().stream().filter(s -> s.getJobProfile().equals(jobProfile)).collect(Collectors.toList());
		 
		//String jpql = "Select new com.app.entities.Job(jobProfile,jobVacancy,experience) j from Job j where j.jobProfile=:jP"; 
		//String jpql = "select r.companyName,r.companyAddress,r.companyContact,j.jobProfile,j.jobVacancy,j.experience from Recruiter r join fetch r.jobs j where j.jobProfile= :jP" ;
	
		String jpql = "select r from Recruiter r join fetch r.jobs j where j.jobProfile= :jP" ; 
		//String jpql = "select r from Recruiter r join  r.jobs j where j.jobProfile= :jP" ;
		//String jpql = "select j from Job j where j.jobProfile= :jP" ; 
		
		
		 List<Recruiter>recruiters = em.createQuery(jpql,Recruiter.class).setParameter("jP", jobProfile).getResultList() ;
        // List<JobSearchResponse>recruiters = em.createQuery(jpql,JobSearchResponse.class).setParameter("jP", jobProfile).getResultList() ;
	...
	public Object applyJob(long jobId,long empId) {
		...
		em.getJobs().add(jobRepo.findById(jobId).orElseThrow(()->new ResourceNotFoundException("Invalid Job Id")));
		}
